...
@Slf4j
@Service
public class OpenAiWhatIfLlmClient implements WhatIfLlmClient {
    ...
    public OpenAiWhatIfLlmClient(
            @Value("${openai.whatif.enabled:true}") boolean enabled,
            @Value("${openai.api-key:}") String apiKey,
            @Value("${openai.whatif.model:${openai.model:gpt-4o-mini}}") String model,
            @Value("${openai.whatif.connect-timeout-ms:1500}") int connectTimeoutMs,
            @Value("${openai.whatif.read-timeout-ms:7000}") int readTimeoutMs
    ) {
    ...
    public Optional<String> generateJson(String prompt) {
        ...
        try {
            ...
            log.warn("What-if LLM generation failed: {}", ex.getMessage());
            return Optional.empty();
        }
    }
